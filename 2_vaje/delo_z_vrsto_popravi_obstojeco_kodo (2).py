...
# =====================================================================@022101=
# 1. podnaloga
# V datoteki je definirana funkcija `prestej_elemente(v)`, ki kot argument
# dobi vrsto `v` in vrne število elementov v vrsti. V definiciji je več napak.
# Odpravi jih. Ko funkcija konča, mora biti vrsta `v` enakem
...
# dobi vrsto `v` in funkcijo `p` (funkcija `p` dobi en argument in vrne
# logično vrednost) ter prešteje, koliko je takih elementov v vrsti, za
# katere funkcija `p` vrne True. V definiciji je več napak. Odpravi jih.
...
# pri 0). Če vrsta ne vsebuje $n$-tega elementa (ker je prekratka),
...
# dobi vrsto `v` in funkcijo `p` (funkcija `p` dobi en argument in vrne
# logično vrednost) ter odstrani tiste elemente v vrsti, za katere funkcija
# `p` vrne `True`. Definicija ima več napak. Odpravi jih.
# 
# Primer:
# 
#     >>> v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjEwMX0:1iL2ov:wkLqLlXrqzuuw5FTtF1CgI7b1-o'
        try:
            ...
            # Check.secret(prestej_elemente(Vrsta([i%40 for i in range(randrange(400, 550))])))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjEwMn0:1iL2ov:yD6Y8FqrsLPpM8nTzrF53XbtnjE'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjEwM30:1iL2ov:a4xN5jAmTIgFYPjH_hnHuz6EF8Y'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "odstrani_ntega(v, 3)",
                       "potem1 = str(v)",
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjEwNH0:1iL2ov:Sh_Ow5zDTpCDgxlnFAU7N85IjH4'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "odstrani_pogojno(v, lambda x: x % 2 == 0)",
                       "potem = str(v)"],
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjEwNX0:1iL2ov:eADmzLO0p0lF8tGaRC39gz1rYe4'
        try:
            Check.run(["v = Vrsta([1, 2, 3, 4, 5, 6, 7, 8])",
                       "obrni(v)",
                       "potem = str(v)"],
