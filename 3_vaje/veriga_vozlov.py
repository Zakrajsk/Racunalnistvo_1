...
def vrni_seznam(prvi):
    """Funkcija vrne seznam, ki predstavlja zaporedje podatkov v verigi vozlov"""
    sez = list()
    poz = prvi
    while poz is not None:
        sez.append(poz.podatek)
        poz = poz.naslednji
    return sez
...
def dodaj_na_zacetek(prvi, x):
    """Funkcija doda nov vozel na začetek verige vozlov, vrne referenco na novi prvi vozel"""
    return Vozel(x, prvi)
...
def dodaj_na_konec(prvi, x):
    """Funkcija doda nov vozel na konec verige vozlov"""
    if prvi is None:
        return Vozel(x)
    poz = prvi
    while poz.naslednji is not None:
        poz = poz.naslednji
    poz.naslednji = Vozel(x)
    return prvi
...
def iz_seznama(seznam):
    """Vrne referenco na zacetek nove verige, ki jo zgenerira iz podanega seznama"""
    if not seznam:
        return None
    prvi = Vozel(seznam[0])
    poz = prvi
    for el in seznam[1:]:
        poz.naslednji = Vozel(el)
        poz = poz.naslednji
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjAzN30:1iNEH5:bOyeZS9_ze3I4NyfKlbeDMxx0qw'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjAzOH0:1iNEH5:HCt3nnVx5LVWumeS9Ezkz3g1ku8'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjAzOX0:1iNEH5:XvsTRXuzw5BVb_SCVS4P455e-wg'
        try:
            test_data = [
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMjA0MH0:1iNEH5:K8oKfGDUODOAsIXzf-H2O7O6heM'
        try:
            test_data = [
                (["v = iz_seznama([])",
                  "l = vrni_seznam(v)"],
