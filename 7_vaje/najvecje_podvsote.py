...
def intervali_najvecjih_vsot_kubicna(sez):
    '''Izračun največje vsote podzaporedja'''
    opt = 0
    n = len(sez)
    mnozica_krajisc = set()
    for i in range(n):
        for j in range(i, n):
            ...
            for k in range(i + 1, j + 1):
                s = s + sez[k]
            if s == opt:
                mnozica_krajisc.add((i, j + 1))
            if s > opt:
                opt = s
                mnozica_krajisc.clear()
                mnozica_krajisc.add((i, j + 1))
    return mnozica_krajisc
# =====================================================================@022762=
# 2. podnaloga
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc2MSwidXNlciI6MjA5OX0:1iaI8I:U6_W_BJ8kAtS-MRIxBUWshgB4PI'
        try:
            Check.equal('intervali_najvecjih_vsot_kubicna([-3, 12, -3])', {(1, 2)})
            Check.equal('intervali_najvecjih_vsot_kubicna([-3, 12, 3, 5, -10, -8, 7, 10])', {(1, 4)})
            Check.equal('intervali_najvecjih_vsot_kubicna([-3, 12, 3, 5, -10, -8, 7, 11])', {(1, 4), (1, 8)})
            Check.equal('intervali_najvecjih_vsot_kubicna([12, 3, 5, -10, -8, 7, 11, -30, 6, 8, -40])', {(0, 3), (0, 7)})
            Check.equal('intervali_najvecjih_vsot_kubicna([2, -2, 12, 3, 5, -10, -8, 7, 11, -30, 6, 8, -40])', {(0, 5), (0, 9), (2, 5), (2, 9)})
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc2MiwidXNlciI6MjA5OX0:1iaI8I:vB8WAQsIWFiCKF9bNj-DA7mames'
        try:
            Check.equal('intervali_najvecjih_vsot_kvadratna([-3, 12, -3])', {(1, 2)})
            Check.equal('intervali_najvecjih_vsot_kvadratna([-3, 12, 3, 5, -10, -8, 7, 10])', {(1, 4)})
            Check.equal('intervali_najvecjih_vsot_kvadratna([-3, 12, 3, 5, -10, -8, 7, 11])', {(1, 4), (1, 8)})
            Check.equal('intervali_najvecjih_vsot_kvadratna([12, 3, 5, -10, -8, 7, 11, -30, 6, 8, -40])', {(0, 3), (0, 7)})
            Check.equal('intervali_najvecjih_vsot_kvadratna([2, -2, 12, 3, 5, -10, -8, 7, 11, -30, 6, 8, -40])', {(0, 5), (0, 9), (2, 5), (2, 9)})
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc2MywidXNlciI6MjA5OX0:1iaI8I:LED52Nb9Ds-bGms8nmiaUbQE_Fc'
        try:
            Check.equal('intervali_najvecjih_vsot_linearna([-3, 12, -3])', {(1, 2)})
            Check.equal('intervali_najvecjih_vsot_linearna([-3, 12, 3, 5, -10, -8, 7, 10])', {(1, 4)})
            Check.equal('intervali_najvecjih_vsot_linearna([-3, 12, 3, 5, -10, -8, -2, 2, 7, 11])', {(1, 4), (7, 10)})
            Check.equal('intervali_najvecjih_vsot_linearna([12, 3, 5, -10, -8, 7, 11, -30, 6, 8, -40])', {(0, 3)})
            Check.equal('intervali_najvecjih_vsot_linearna([2, -2, 12, 3, 5, -10, -8, 7, 11, -30, 6, 8, -40])', {(2, 5)})
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
