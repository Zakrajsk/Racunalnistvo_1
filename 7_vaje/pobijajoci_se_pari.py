...
def nicelni_pari_v_urejenem(seznam):
    """Vrne true, ce sta v seznamu dve stevili katerih vsota je 0, pri tem je seznam urejen"""
    obratni = set()
    for i in range(len(seznam) // 2):
        zacetek = seznam[i]
        konec = seznam[-i - 1]
        if zacetek in obratni or konec in obratni or zacetek + konec == 0:
            return True
        obratni.update([-zacetek, -konec])
    return False
...
def nicelni_pari_v_neurejenem(seznam):
    """Vrne true, ce sta v seznamu dve stevili katerih vsota je 0, pri tem je seznam neurejen"""
    obratni = set()
    for el in seznam:
        if el in obratni:
            return True
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc3MSwidXNlciI6MjA5OX0:1iaH4w:MzNHKf3TQ2OiWqBr_JSJbMxoItw'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc3MiwidXNlciI6MjA5OX0:1iaH4w:i42snlR56U2bbNpP2Ct3AjOw6jQ'
        try:
