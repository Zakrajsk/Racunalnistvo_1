...
def najvecji_podprodukt(sez):
    """Izračuna največjo podprodukt strnjenega podzaporedja števil v seznamu"""
    trenutni_najboljsi = 0
    zacasnji_produkt = 1
    for el in sez:
        naslednji_produkt = zacasnji_produkt * el
        if (naslednji_produkt > 1):
            if (naslednji_produkt > trenutni_najboljsi) :
                trenutni_najboljsi = naslednji_produkt
            zacasnji_produkt = naslednji_produkt
        else :
            zacasnji_produkt = 1
    return trenutni_najboljsi
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMjc2MCwidXNlciI6MjA5OX0:1iaGzI:PnT61HrlvR2Ee8bjclFlmKH3HTY'
        try:
