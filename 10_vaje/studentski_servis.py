...
def tabele01(n):
    """Funckija vrne tabelo vseh moznih tabel dolzine n, ki so sestavljene iz samih nicel in enk"""
    vse_moznosti = list()
    for i in range(n + 1):
        vse_moznosti += list(map(list, set(itertools.permutations([1] * i + [0] * (n - i)))))
    return sorted(vse_moznosti)

def zasluzek(opravila, cas):
    """Pregleda vse mozne kombinacije izbire opravil in vrne najvecji mozni zasluzek"""
    vse_kombinacije = tabele01(len(opravila))
    casi_opravil = [el[0] for el in opravila]
    zasluzki_opravil = [el[1] for el in opravila]
    najvecji_zasluzek = 0
    for kombinacija in vse_kombinacije:
        skupen_cas = sum([(da_ali_ne * cas) for da_ali_ne, cas in zip(kombinacija, casi_opravil)])
        if skupen_cas <= cas:
            skupen_zasluzek = sum([(da_ali_ne * denar) for da_ali_ne, denar in zip(kombinacija, zasluzki_opravil)])
            if skupen_zasluzek > najvecji_zasluzek:
                najvecji_zasluzek = skupen_zasluzek
    return najvecji_zasluzek
...
def katera(opravila, cas):
    ...
    zasluzki_opravil = [el[1] for el in opravila]
    najvecji_zasluzek = 0
    najboljsa_kombinacija = [0] * len(opravila)
    for kombinacija in vse_kombinacije:
        skupen_cas = sum([(da_ali_ne * cas) for da_ali_ne, cas in zip(kombinacija, casi_opravil)])
        if skupen_cas <= cas:
            skupen_zasluzek = sum([(da_ali_ne * denar) for da_ali_ne, denar in zip(kombinacija, zasluzki_opravil)])
            if skupen_zasluzek > najvecji_zasluzek:
                najvecji_zasluzek = skupen_zasluzek
                najboljsa_kombinacija = kombinacija
    return najboljsa_kombinacija
# =====================================================================@023295=
# 3. podnaloga
...
def zasluzek_bellman(opravila, cas):
    """z bellmanovo enacbo rekurzivno resi problem,
     kaksen je najboljsi mozen zasluzek,
     glede na podana opravila in zasluzke le teh"""
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMzI5MywidXNlciI6MjA5OX0:1ihu4A:kEVwi-JXVF3Sy04ohfEChWe6hn8'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMzI5NCwidXNlciI6MjA5OX0:1ihu4A:BZK7RtwpOuN5KVlxd1s_Ihsjnqo'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMzI5NSwidXNlciI6MjA5OX0:1ihu4A:qHunxicH8XbtyRlXPz6JAnjJDpM'
        try:
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJwYXJ0IjoyMzI5NiwidXNlciI6MjA5OX0:1ihu4A:lb-Bm7f3RQkUJWplrW8raZl0sYM'
        try:
