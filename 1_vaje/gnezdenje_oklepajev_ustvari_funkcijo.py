...
def oklepaji(niz):
    ...
    for znak in niz:
        if znak in oklep.keys():
            ...
            if sklad_oklepajev.prazen() or znak != oklep[sklad_oklepajev.vrh()]:
                return False
            sklad_oklepajev.odstrani()
    return sklad_oklepajev.prazen()
# =====================================================================@021859=
# 2. podnaloga
# Definirana je funkcija `max_globina(niz)`, ki bi morala poiskati
# globino najbolj globoko gnezdenega para oklepajev v izrazu `niz`. Funkcija
# vsebuje več napak. Najprej jo dopolni do funkcije iz prejšnje naloge, nato
# pa odpravi še preostale napake. Končna funkcija naj torej v primeru pravilno
...
def _validate_current_file():
    ...
    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMTg1OH0:1iIVbj:HirAIpYFTbobZlKwGfe-va5n06E'
        try:
            test_data = [
            ...
            Check.secret(oklepaji('3-(2-y)*[1+4**(3+1)]<()>'))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])

    if Check.part():
        Check.current_part['token'] = 'eyJ1c2VyIjoyMDk5LCJwYXJ0IjoyMTg1OX0:1iIVbj:5RHbCWW4jWY5WdlmH4uSbpYFY74'
        try:
            test_data = [
                ("max_globina('burek')", 0),
            ...
            Check.secret(max_globina('3-(2-y)*[1+4**(3+1)]<(>'))
            Check.secret(max_globina('3-(2-y)*[1+4**(3+1)]<([{3x}+2])>'))
        except:
            Check.error("Testi sprožijo izjemo\n  {0}",
                        "\n  ".join(traceback.format_exc().split("\n"))[:-2])
